		...
		if (defaultFailureUrl == null) {
			...
			if (forwardToDestination) {
				...
				if(true) {
					...
					Map map = new HashMap<>();
					map.put("login_flag", "false");
					map.put("error_msg", exception.getMessage());
					map.put("error_class", exception.getClass());
					mapper.writerWithView(String.class).writeValueAsString(map);

	        		try {
	        			PrintWriter e1 = response.getWriter();
	        			e1.print(mapper.writerWithView(String.class).writeValueAsString(map));
	        			e1.close();
